...
export const Todo = ({ todo: { id, title, description, deadline, completed, disabled }, onChangeTodo, onDeleteTodo }) => {
    ...
    return (
        <div data-testid="todo-item" className="todo" key={id}>
            <div className="deadline">
                {deadline}
                {isExpired() && <Chip style={{ marginLeft: "4px" }} size="small" label="Expired" variant="outlined" />}</div>
            <div className="title">{title}</div>
            <div className="actions">
                <Checkbox data-testid="completed" checked={completed} disabled={disabled || isExpired()} onChange={() => onChangeTodo({ id, completed: !completed })} />
